    ...
    <div className="bg-white rounded-2xl shadow-[0px_6px_20px_rgba(0,0,0,0.05)] border-2 border-black p-6 mb-6 animate-[slideDown_0.3s_ease-out]">
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1">
          <h3 className="text-lg font-bold text-[#111111] mb-1">{title || 'Client account successfully created'}</h3>
          <p className="text-sm text-[#6B7280] mb-4">
            {message || (
              <>
                The new client account has been set up and is ready for integration. Client ID:{' '}
                <span className="font-mono font-semibold text-[#111111]">{clientId}</span>
              </>
            )}
          </p>
          <div className="flex items-center gap-3">
            <Link 
              href={linkHref || `/dashboard/clients/${clientId}`}
